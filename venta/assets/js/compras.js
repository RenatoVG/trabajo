...
document.addEventListener('DOMContentLoaded', function () {
  $('#table_compra').DataTable({
    ajax: {
      url: ruta + 'presenter/comprasPresenter.php?option=listar',
      dataSrc: ''
    },
    columns: [
      { data: 'codigo' },
  ...
  table_proveedores = $('#table_proveedores').DataTable({
    ajax: {
      url: ruta + 'presenter/comprasPresenter.php?option=listar-proveedores',
      dataSrc: ''
    },
    columns: [
      { data: 'nombre' },
  ...
  btn_save.onclick = function () {
    axios.post(ruta + 'presenter/comprasPresenter.php?option=savecompra', {
  ...
  seacrh.onkeyup = function (e) {
    if (e.key === "Enter") {
      if (e.target.value == '') {
        ...
        axios.get(ruta + 'presenter/comprasPresenter.php?option=searchbarcode&barcode=' + e.target.value)
          .then(function (response) {
            const info = response.data;
            seacrh.value = '';
            message(info.tipo, info.mensaje);
...
function addCart(codProducto) {
  axios.get(ruta + 'presenter/comprasPresenter.php?option=addcart&id=' + codProducto)
    .then(function (response) {
      const info = response.data;
      message(info.tipo, info.mensaje);
      temp();
...
function temp() {
  axios.get(ruta + 'presenter/comprasPresenter.php?option=listarTemp')
    .then(function (response) {
      const info = response.data;
      let tempProductos = '';
      info.forEach(pro => {
...
function addCantidad(e, idTemp) {
  axios.post(ruta + 'presenter/comprasPresenter.php?option=addcantidad', {
    id: idTemp,
...
function addPrecio(e, idTemp) {
  axios.post(ruta + 'presenter/comprasPresenter.php?option=addprecio', {
    id: idTemp,
...
function deleteproducto(idTemp) {
  axios.get(ruta + 'presenter/comprasPresenter.php?option=delete&id=' + idTemp)
    .then(function (response) {
      const info = response.data;
      message(info.tipo, info.mensaje);
      temp();
