...
document.addEventListener('DOMContentLoaded', function () {
  $('#table_venta').DataTable({
    ajax: {
      url: ruta + 'presenter/ventasPresenter.php?option=listar',
      dataSrc: ''
    },
    columns: [
      { data: 'codigo' },
  ...
  table_clientes = $('#table_clientes').DataTable({
    ajax: {
      url: ruta + 'presenter/ventasPresenter.php?option=listar-clientes',
      dataSrc: ''
    },
    columns: [
      { data: 'nombre' },
  ...
  btn_save.onclick = function () {
    axios.post(ruta + 'presenter/ventasPresenter.php?option=saveventa', {
  ...
  seacrh.onkeyup = function (e) {
    if (e.key === "Enter") {
      if (e.target.value == '') {
        ...
        axios.get(ruta + 'presenter/ventasPresenter.php?option=searchbarcode&barcode=' + e.target.value)
          .then(function (response) {
            const info = response.data;
            seacrh.value = '';
            message(info.tipo, info.mensaje);
...
function addCart(codProducto) {
  axios.get(ruta + 'presenter/ventasPresenter.php?option=addcart&id=' + codProducto)
    .then(function (response) {
      const info = response.data;
      message(info.tipo, info.mensaje);
      temp();
...
function temp() {
  axios.get(ruta + 'presenter/ventasPresenter.php?option=listarTemp')
    .then(function (response) {
      const info = response.data;
      let tempProductos = '';
      info.forEach(pro => {
...
function addCantidad(e, idTemp) {
  axios.post(ruta + 'presenter/ventasPresenter.php?option=addcantidad', {
    id: idTemp,
...
function addPrecio(e, idTemp) {
  axios.post(ruta + 'presenter/ventasPresenter.php?option=addprecio', {
    id: idTemp,
...
function deleteproducto(idTemp) {
  axios.get(ruta + 'presenter/ventasPresenter.php?option=delete&id=' + idTemp)
    .then(function (response) {
      const info = response.data;
      message(info.tipo, info.mensaje);
      temp();
