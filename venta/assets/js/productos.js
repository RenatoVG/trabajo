...
document.addEventListener('DOMContentLoaded', function () {
  $('#table_productos').DataTable({
    ajax: {
      url: ruta + 'presenter/productosPresenter.php?option=listar',
      dataSrc: ''
    },
    columns: [
      { data: 'codproducto' },
  ...
  frm.onsubmit = function (e) {
    ...
    if (barcode.value == '' || nombre.value == ''
      ...
      axios.post(ruta + 'presenter/productosPresenter.php?option=save', frmData)
...
function deleteProducto(id) {
  Snackbar.show({
    ...
    onActionClick: function (element) {
      axios.get(ruta + 'presenter/productosPresenter.php?option=delete&id=' + id)
        .then(function (response) {
          const info = response.data;
          message(info.tipo, info.mensaje);
          if (info.tipo == 'success') {
...
function editProducto(id) {
  axios.get(ruta + 'presenter/productosPresenter.php?option=edit&id=' + id)
    .then(function (response) {
      const info = response.data;
